...
def grant_amount_heatmap(df, grouped_df, selected_chart, selected_role, ai_enabled):
    ...
    with col_main:
        ...
        with st.expander("Heatmap Settings", expanded=False):
            ...
            with colA:
                colorscale = st.selectbox(
                    "Colorscale",
                    ["Plasma", "Viridis", "Cividis", "Inferno", "Magma", "Blues", "Greens", "Reds"],
                    index=0,
                )
            with colB:
        ...
        # Persist heatmap filter state for AI chart-state tool
        try:
            st.session_state["heatmap_dimension1"] = dimension1
            st.session_state["heatmap_dimension2"] = dimension2
            st.session_state["heatmap_values1"] = list(selected_values1)
            st.session_state["heatmap_values2"] = list(selected_values2)
            st.session_state["heatmap_normalize"] = bool(normalize)
            st.session_state["heatmap_colorscale"] = str(colorscale)
        except Exception:
            pass
        ...
        # Additional exploration (remains in main column)
        ...
        # Prepare context string for chat (used in right column)
        additional_context = (
            f"the intersection of {dimension1.split('_')[1]} and {dimension2.split('_')[1]} dimensions, "
    ...
    if ai_enabled:
        ...
        _ = st.sidebar.selectbox(
            "Chat about",
            options=["Heatmap Chart"],
            index=0,
            key="heatmap_chat_target",
        )
