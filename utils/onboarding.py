...
from utils.app_state import UserProfile, role_label
...
class OnboardingWizard:
    ...
    def render(self, profile: UserProfile | None = None) -> tuple[UserProfile | None, bool]:
        """
        Render the onboarding wizard.
        Returns (profile, completed) tuple.
        """
        # Onboarding is enabled by default (no feature flag gate)
        ...
        with col2:
            ...
            if st.button("Continue →", type="primary", disabled=not can_continue):
                st.session_state.onboarding_data.update(
                    {"org_type": org_type, "primary_goal": primary_goal or "", "region": region}
                )
                return True
