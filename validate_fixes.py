...
from advisor.schemas import StructuredNeeds, DataPoint
...
def test_funder_candidates():
    ...
    data = pd.DataFrame({
        'funder_name': ['Ford Foundation', 'National Science Foundation', 'Bill & Melinda Gates Foundation', 'Hewlett Foundation', 'Robert Wood Johnson Foundation', 'Packard Foundation', 'Surdna Foundation', 'Kresge Foundation', 'Mott Foundation', 'Rockefeller Foundation'],
        'amount_usd': [500000, 750000, 650000, 480000, 820000, 420000, 380000, 450000, 520000, 610000],
        'year_issued': ['2023'] * 10
    })
    
    print(f"Test data has {len(data)} funders with amounts ranging from ${data['amount_usd'].min():,} to ${data['amount_usd'].max():,}")
...
def test_section_generation():
    ...
    datapoints = [
        {'id': 'DP-001', 'title': 'Funding Analysis', 'method': 'df_pivot_table', 'params': {}, 'table_md': '| Metric | Value |\n|--------|-------|\n| Avg Amount | $525K |', 'notes': 'Test analysis'},
        {'id': 'DP-002', 'title': 'Geographic Trends', 'method': 'df_groupby_sum', 'params': {}, 'table_md': '| Region | Funding |\n|--------|----------|\n| CA | $2.1M |', 'notes': 'Regional analysis'}
    ]
