...
def onboarding_wizard():
    ...
    if not st.session_state.onboarding_complete:
        ...
        experience = st.radio(
            "My experience level:",
            ["Never", "A little", "Some experience", "Very experienced"]
        )
        ...
        st.success(f"Great! You're looking for {profile.get('budget_range', 'funding')} for {profile.get('project_type', 'your project')}")
...
def smart_recommendations(df: pd.DataFrame):
    ...
    if len(df) > 0:
        ...
        if len(local_funders) > 0:
            st.success(f"💡 **Local Opportunity Found!** {len(local_funders)} funders support local projects like yours.")
        
        # Budget guidance
        avg_amount = df['amount_usd'].median()
        st.info(f"💰 **Budget Reality Check:** Half of grants in your area are under ${avg_amount:,.0f}")
        
        # Timeline guidance  
        recent_grants = df[df['year_issued'] >= 2023]
        if len(recent_grants) > 0:
            st.warning(f"⏰ **Timing Insight:** {len(recent_grants)} recent grants show funders are actively giving now!")
