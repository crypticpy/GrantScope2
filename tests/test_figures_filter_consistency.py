...
from advisor.pipeline.orchestrator import run_interview_pipeline
...
def test_figures_align_with_filtered_data():
    ...
    df = pd.DataFrame(
        [
            {"funder_name": "Local Funder A", "amount_usd": 100000, "grant_geo_area_tran": "Austin", "year_issued": 2023},
            {"funder_name": "Local Funder B", "amount_usd": 50000, "grant_geo_area_tran": "Austin", "year_issued": 2023},
            {"funder_name": "Other Funder", "amount_usd": 200000, "grant_geo_area_tran": "Dallas", "year_issued": 2023},
        ]
    )
    interview = InterviewInput(program_area="Youth education", geography=["TX"], keywords=["education"], user_role="Municipal Program Manager")
