...
def analyze_budget(budget_lines: List[BudgetLine], indirect_rate: float, match_available: bool) -> Dict:
    ...
    if total > 0:
        staff_amount = sum(line.amount for line in budget_lines if line.category.lower() in {"staff", "personnel"})
...
def main():
    ...
    if not require_flag("GS_ENABLE_NEW_PAGES", "Budget Reality Check is not available"):
        return
    ...
    with st.form("budget_form"):
        ...
        num_lines = st.number_input("How many budget lines?", min_value=1, max_value=20, value=4, step=1)
        ...
        with col1:
            indirect_rate = st.number_input("Indirect rate (%)", min_value=0.0, max_value=60.0, value=10.0, step=1.0)
        with col2:
            match_available = st.selectbox("Do you have match funding?", options=["No", "Yes"]) == "Yes"
        with col3:
