...
BUILT_IN_STORIES = [
    {
        ...
        "key_success_factors": ["Clear community need", "Simple, realistic budget", "Strong community support"],
    ...
    },
    {
        ...
        "key_success_factors": ["Business-like planning", "Scalable impact", "Aligned funder values"],
...
def filter_stories(stories: List[Dict], org_type_filter: Optional[str] = None, region_filter: Optional[str] = None) -> List[Dict]:
...
def render_story_card(story: Dict, show_details: bool = False) -> None:
    ...
    with st.container():
        ...
        # Story content
        if show_details or st.button(f"Read Full Story", key=f"story_{hash(story['title'])}"):
...
def main():
    ...
    if not require_flag("GS_ENABLE_NEW_PAGES", "Success Stories is not available"):
        return
    ...
    with col1:
        org_types = ["All"] + sorted(list(set(s.get("org_type", "").title() for s in stories if s.get("org_type"))))
        org_filter = st.selectbox("Organization Type", options=org_types)
    
    with col2:
        regions = ["All"] + sorted(list(set(s.get("region", "") for s in stories if s.get("region"))))
        region_filter = st.selectbox("Region", options=regions)
    
    with col3:
        amount_ranges = ["All"] + sorted(list(set(s.get("amount_range", "") for s in stories if s.get("amount_range"))))
        amount_filter = st.selectbox("Grant Amount", options=amount_ranges)
    
    # Filter stories
    filtered_stories = stories
    if org_filter != "All":
        filtered_stories = [s for s in filtered_stories if s.get("org_type", "").title() == org_filter]
    if region_filter != "All":
        filtered_stories = [s for s in filtered_stories if s.get("region", "") == region_filter]
    if amount_filter != "All":
        filtered_stories = [s for s in filtered_stories if s.get("amount_range", "") == amount_filter]
    
    # Show results
    st.markdown(f"**Found {len(filtered_stories)} stories** {f'(filtered from {len(stories)} total)' if len(filtered_stories) < len(stories) else ''}")
