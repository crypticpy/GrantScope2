...
def generate_timeline_plan(
    ...
    milestones.append({
        ...
        "weeks_before": (draft_weeks + (2 if review_needs else 1) + 2 + research_weeks + planning_weeks),
...
def main():
    ...
    if not require_flag("GS_ENABLE_NEW_PAGES", "Timeline Advisor is not available"):
        return
    ...
    with tab3:
        ...
        if st.session_state.get("timeline_data"):
            ...
            with col1:
                # Text export
                calendar_text = generate_calendar_export(
                    data['milestones'], 
                    data['project_name']
                )
...
"""
                for milestone in data['milestones']:
                    quick_format += f"• {milestone['date'].strftime('%m/%d')} - {milestone['milestone']}\n"
