...
def test_recommendations_realistic_rankings():
    ...
    if len(candidates) > 1:
        ...
        ), f"Scores not properly ordered: {scores}"
    ...
    for i, candidate in enumerate(candidates):
        ...
        if not (candidate.rationale and len(candidate.rationale) > 20):
            failed_candidates.append(
                f"  {candidate.name}: Rationale too short ({len(candidate.rationale) if candidate.rationale else 0} chars)"
            )
            continue

        # Check for funder/foundation keyword
        has_funder = (
            "funder" in candidate.rationale.lower() or "foundation" in candidate.rationale.lower()
        )
        if not has_funder:
        ...
        if not has_data:
            failed_candidates.append(
                f"  {candidate.name}: Missing data keywords. Rationale: '{candidate.rationale}'"
            )
