...
def treemaps_extended_analysis(df, grouped_df, selected_chart, selected_role, ai_enabled):
    if selected_chart == "Treemaps with Extended Analysis":
        ...
        with col1:
            analyze_column = st.radio(
                "Select Variable for Treemap",
        ...
        grouped_data = (
            filtered_data.groupby(analyze_column)["amount_usd"]
            .sum()
            .reset_index()
            .sort_values(by="amount_usd", ascending=True)
        )
        ...
        try:
            ...
            render_ai_explainer(
                grouped_data, pre_prompt, chart_id="treemaps.main", sample_df=grouped_data
            )
        except Exception:
            pass
        ...
        if selected_block:
            ...
            if not block_grants.empty:
                ...
                with col3:
                    st.metric(
                        label="Average Grant USD",
                        value=f"${block_grants['amount_usd'].mean():,.2f}",
                    )

                top_funders = (
                    block_grants.groupby("funder_name")["amount_usd"]
                    .sum()
                    .nlargest(5)
                    .reset_index()
                )
                ...
                top_recipients = (
                    block_grants.groupby("recip_name")["amount_usd"].sum().nlargest(5).reset_index()
                )
                top_recipients["amount_usd"] = top_recipients["amount_usd"].apply(
                    lambda x: f"${x:,.2f}"
                )
