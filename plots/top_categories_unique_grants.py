...
from utils.utils import download_multi_sheet_excel, generate_page_prompt
    ...
    if selected_chart == "Top Categories by Unique Grant Count":
        ...
        with col1:
            selected_categorical = st.selectbox(
                "Select Categorical Variable", options=key_categorical_columns
            )
            top_n = st.slider(
                "Number of Top Categories", min_value=5, max_value=20, value=10, step=1
            )

        with col2:
            chart_type = st.selectbox(
                "Select Chart Type", options=["Bar Chart", "Pie Chart", "Treemap"]
            )
            sort_order = st.radio("Sort Order", options=["Descending", "Ascending"])

        normalized_counts = (
            df.groupby(selected_categorical)["grant_key"]
            .nunique()
        ...
        with expander:
            selected_category = st.selectbox(
                f"Select {selected_categorical} Category",
                options=normalized_counts[selected_categorical],
            )

            category_grants = df[df[selected_categorical] == selected_category].drop_duplicates(
                subset=["grant_key"]
            )
        ...
        if st.button("Download Data for Chart"):
            sheets = {
                ...
                "Grants for Selected Category": (
                    category_grants if "category_grants" in locals() else normalized_counts.head(0)
                ),
            }
