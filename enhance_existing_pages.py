...
def enhance_data_summary_for_newbies(df: pd.DataFrame, grouped_df: pd.DataFrame, selected_role: str):
    ...
    top_funders = (
        unique_df.groupby("funder_name")["amount_usd"]
    ...
    if len(funder_type_dist) > 8:
        ...
        other_sum = funder_type_dist.nsmallest(len(funder_type_dist)-7, 'amount_usd')['amount_usd'].sum()
...
def create_newbie_friendly_page(df: pd.DataFrame, page_type: str):
    ...
    if page_type == "data_summary":
        ...
        enhance_grant_distribution_for_newbies(df, st.session_state.get('user_role', 'Normal Grant User'))
...
if __name__ == "__main__":
    ...
    sample_data = pd.DataFrame({
        ...
        'funder_name': ['Community Foundation', 'Local Arts Council', 'City Grant Program', 'State Agency', 'Corporate Giving'] * 20,
        'funder_type': ['Foundation', 'Community Foundation', 'Government', 'Government', 'Corporate'] * 20,
